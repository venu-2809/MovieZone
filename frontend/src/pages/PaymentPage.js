...
const PaymentPage = () => {
  ...
  const handlePayment = async () => {
    ...
    // Simulate payment gateway success (replace with real gateway in production)
    setTimeout(async () => {
      try {
        // Call backend to mark as paid
        await axios.post(
          `http://localhost:5000/api/payments/pay/${movieId}`,
          {},
          {
        ...
        // Redirect to movie page after short delay
        setTimeout(() => navigate(`/movie/${movieId}`), 1800);
      } catch (err) {
        setIsPaying(false);
