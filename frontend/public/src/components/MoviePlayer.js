...
const MoviePlayer = ({ movieUrl, username }) => {
  ...
  useEffect(() => {
    if (!username) return;
    const moveWatermark = () => {
      // Video area: adjust these ranges as needed for your layout
      const top = Math.floor(Math.random() * 70) + 10; // 10px to 80px
      const left = Math.floor(Math.random() * 70) + 10; // 10px to 80px
  ...
  // Keep seekValue in sync with currentTime when not seeking
  useEffect(() => {
    if (!isSeeking) setSeekValue(currentTime);
  }, [currentTime, isSeeking]);

  // Pause video on unmount to avoid play() interruption error
  useEffect(() => {
    return () => {
      if (videoRef.current) {
      ...
      >
        ...
        <video
          ...
          style={{
            ...
            height: isFullscreen ? '100vh' : getVideoHeight(),
            maxHeight: isFullscreen ? '100vh' : getVideoHeight(),
            background: '#000',
            display: 'block',
            objectFit: 'contain',
            borderRadius: 0,
